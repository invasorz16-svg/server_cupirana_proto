...
# ── ARCHIVO DE TOKEN (el usuario lo pega ahi)
...
try:
    ...
    # Si __file__ no esta definido, usar el directorio actual
...
class API_Client:
    ...
    def _get_token(self):
        """Devuelve el token activo, leyendo del archivo si es necesario."""
        with _token_lock:
    ...
    def invalidate_token(self):
        """Marca el token como invalido para forzar relectura."""
        with _token_lock:
            _current_token["value"] = None
    ...
    def req(self, method, path, body=None):
        ...
        try:
            with urllib.request.urlopen(r, timeout=15) as resp:
                ...
                # Si es lista, envolverla para mantener consistencia
...
class VMCDesktop:
    ...
    def _build(self):
        ...
        self._build_log()
        # Refrescar items cuando se cambia a esa pestana
        self.nb.bind("<<NotebookTabChanged>>", self._on_tab_change)

    def _on_tab_change(self, event):
        try:
            current = self.nb.select()
            tab_name = self.nb.tab(current, "text")
            if "Articulos" in tab_name or "rtículos" in tab_name:
                # Recargar pedidos primero, luego renderizar items
    ...
    def _render_items(self):
        """Articulos vendidos con filtro de periodo y resumen agrupado."""
        now_tz = datetime.now(TZ)
        today_date = now_tz.date()
        mode  = self._items_mode.get()
        items = []
        self._log(f"Filtrando articulos: modo={mode}, hoy={today_date}, total_pedidos={len(self.orders)}", "in")

        for o in self.orders:
            # Parsear fecha (viene en hora local de la maquina, ya en TZ -06:00)
            ...
            fail  = det.get("failureNum", 0) or 0
            state = "Entregado" if succ>0 else ("Fallo" if fail>0 else "Pendiente")
            # SOLO contar ventas reales (Entregadas) — ignorar Pendientes y Fallos
            if state != "Entregado":
                continue
            items.append({
...
#  MAIN
# ══════════════════════════════════════════════════════════
def _show_error_window(error_msg):
    """Muestra una ventana con el error completo si algo falla."""
    err_root = tk.Tk()
    err_root.title("VMC Control Center - Error")
    err_root.geometry("720x520")
    err_root.configure(bg="#0a0d13")
    tk.Label(err_root, text="Error al iniciar la aplicacion",
             bg="#0a0d13", fg="#ff5c72",
             font=("Segoe UI", 14, "bold")).pack(pady=14)
    txt = scrolledtext.ScrolledText(err_root, bg="#171c2a", fg="#ccd6f6",
    ...
    tk.Button(err_root, text="Cerrar", command=err_root.destroy,
              bg="#ff5c72", fg="#fff", font=("Segoe UI", 10, "bold"),
...
if __name__ == "__main__":
    import traceback
    try:
        # ── Usar UN SOLO Tk() y reciclarlo entre login y app ──
        # Si ya hay un token guardado, saltarse el login
        existing = load_token_from_file()

        if existing:
            # Validar formato basico
            jwt_part = existing[7:] if existing.startswith("Bearer ") else existing
            if jwt_part.count(".") == 2 and len(jwt_part) >= 50:
                # Token presente, ir directo a la app
                with _token_lock:
                    _current_token["value"] = existing
                root = tk.Tk()
                app  = VMCDesktop(root)
        ...
        # No hay token o es invalido → mostrar login primero
        login_root = tk.Tk()
        ...
        # Login completado → abrir la app
        # Importante: crear el nuevo Tk() limpio
        root = tk.Tk()
        app  = VMCDesktop(root)
